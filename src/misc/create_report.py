...
def replace_fields(html, patient_name, nmr_number, scan_date):
    ...
    html = html.replace('NMR number', nmr_number)
...
def main(html_template_fname, scan_fol, patient_name, nmr_number, scan_date, output_fname=''):
    if utils.get_parent_fol(html_template_fname) != scan_fol:
        shutil.copy(html_template_fname, scan_fol)
    if output_fname == '':
        output_fname = op.join(scan_fol, '{}.pdf'.format(nmr_number))
    new_html_fname =  utils.change_fname_extension(output_fname, 'html')
    html = read_html(html_template_fname)
    html = replace_fields(html, patient_name, nmr_number, scan_date)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--template_fname', required=True)
    parser.add_argument('--scan_fol', required=True)
    parser.add_argument('--patient_name', required=True)
    parser.add_argument('--nmr_number', required=True)
    parser.add_argument('--scan_date', required=True)
    args = utils.Bag(au.parse_parser(parser))
    main(args.template_fname, args.scan_fol, args.patient_name, args.nmr_number, args.scan_date)
